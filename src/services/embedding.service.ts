...
import { LocalEmbeddingService } from './embeddings/local-embedding.service.js';
...
export class EmbeddingService {
   ...
   */
  private validateEmbeddingModel(model: string): void {
  ...
  private async generateLocalEmbedding(text: string): Promise<EmbeddingResult> {
    ...
    const embedding = Array.from({ length: vectorSize }, (_, i) =>
      Math.sin(hash + i) * 0.1 + Math.cos(hash * 2 + i) * 0.05
    );
   ...
   */
  private calculateEmbeddingCost(model: string, tokens: number): EmbeddingResult['cost'] {
   ...
   */
  private async generateAPIEmbedding(
    text: string,
    options?: EmbeddingOptions
  ): Promise<EmbeddingResult> {
    if (!this.apiKey) {
