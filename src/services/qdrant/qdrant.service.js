...
export class QdrantService {
     ...
     */
    getVectorSizeForModel(model) {
        // OpenAI models
        if (model.includes('text-embedding-3-large'))
            return 3072;
        if (model.includes('text-embedding-3-small') || model.includes('text-embedding-ada-002'))
            return 1536;
        if (model.includes('text-embedding-ada-001'))
            return 768;
        // Voyage AI models
        if (model.includes('voyage-3-large'))
            return 1024;
        if (model.includes('voyage-3'))
            return 1024;
        if (model.includes('voyage-2'))
            return 1024;
        if (model.includes('voyage-code-2'))
            return 1536;
        // Cohere models
        if (model.includes('embed-english-v3.0') || model.includes('embed-multilingual-v3.0'))
            return 1024;
        if (model.includes('embed-english-v2.0'))
            return 4096;
        if (model.includes('embed-english-light-v2.0'))
            return 1024;
        // Default fallback
     ...
     */
    async searchHybrid(denseVector, sparseVector, options) {
     ...
     */
    async searchDense(denseVector, options) {
        const topK = options?.topK || 5;
